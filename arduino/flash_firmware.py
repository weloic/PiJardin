...
# read_puit.py lives in sensors/, alerts.py in telegram_bot/; when run as a script,
# only arduino/ is on sys.path (same bootstrapping pattern as bot.py / read_puit.py).
...
def systemctl(action, unit):
    ...
    try:
        ...
        print(f"WARNING: systemctl {action} {unit} failed: {e}")
...
def enter_bootloader():
    ...
    try:
        ...
        time.sleep(0.1)
        s.close()
        print("1200-baud touch sent; board should reset into the bootloader.")
    except (serial.SerialException, OSError) as e:
        print(f"1200-baud touch could not open {PORT} ({e}); "
